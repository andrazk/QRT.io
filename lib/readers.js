...
readers.readDataPacket = function(data){
	...
	//var typ = data.readInt32BE(4);
    // time stamp
    var int64TS = new Int64('0x' + data.toString('hex', 8, 16));

	var out = {
		timestamp: parseInt(int64TS.toUnsignedDecimalString(), 10),
 ...
 * @param  {[type]} offset whre to start reading
 *
 * @return {mixed}
 */
readers.readMarkers = function (buffer, offset) {

    offset = offset || 16;

    var markers = {
        x: buffer.readFloatBE(offset),
        y: buffer.readFloatBE(offset + 4),
        z: buffer.readFloatBE(offset + 8)
    };
...
readers.read3DComponent = function (data, _l, _t) {
	...
	for (var i = 0; i < count; i++) {
        ...
        */
        out.markers[i] = readers.readMarkers(data, offset);

        if (out.markers[i] === false) {
            // sth is wrong with data
            out.markers[i] = null;
            continue;
